...
// 1. CORS Setup (SAHI ORIGIN KE SATH)
app.use(cors({
  origin: [
    'https://sparkly-pony-d794c3.netlify.app', // Yahan koi space nahi hona chahiye
    'http://localhost:5173'
  ],
  credentials: true,
...
// 2. Body Parsers (Routes se pehle hone chahiye)
app.use(express.json());
app.use(express.urlencoded({ extended: true }));

app.use('/uploads', express.static(path.join(__dirname, 'uploads')));

// 3. Health check aur Root route
app.get('/', (req, res) => {
  res.send('Smart AI Academic System Backend is Running!');
});
